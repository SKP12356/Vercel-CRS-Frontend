...
const OtpVerifyMail = () => {
  const { provideOtp, otpResend, providePhoneOtp } = useContext(CarContext);
  ...
  const sendOtp = async () => {
    ...
    console.log(otp);
    const data = await provideOtp(id, otp);
    console.log(data);
    setLoad(false);
    if (data?.message) {
  ...
  return (
    <>
      {load === false ? (<div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-indigo-50 to-blue-100 p-4">
